...
const fmt = (value, digits = 4) => {
    ...
    if (abs !== 0 && abs < 0.0001) {
        ...
        return decimalSeparator === "," ? exp.replace(".", ",") : exp;
    }
...
/**
 * Normalise a single numeric token into a JavaScript number.
 * The logic is FULLY determined by the global `decimalSeparator` setting:
 *
 *  decimalSeparator = ","  →  comma is decimal, dot is thousands  (pt-BR)
 *  decimalSeparator = "."  →  dot   is decimal, comma is thousands (en-US)
 *
 * Returns NaN for anything that cannot be reliably interpreted.
 */
function normalizeToken(token) {
    ...
    if (decimalSeparator === ",") {
        // pt-BR mode: comma = decimal, dot = thousands
        ...
        // if only dots: 72.5 is treated as 72.5 (thousands separator not applicable for small numbers)
        // This is fine — a dot in pt-BR mode with no comma is unambiguous (thousands or already a JS number)
    } else {
        // en-US mode: dot = decimal, comma = thousands
        if (t.includes(",") && t.includes(".")) {
            // e.g. 1,234.56  →  remove commas
            t = t.replace(/,/g, "");
        } else if (t.includes(",")) {
            // Comma with no dot: reject/treat as thousands? 
            // In en-US mode, a lone comma like "72,5" is ambiguous.
            // We treat it as thousands separator ("725") ONLY if the part after comma is 3 digits.
            // Otherwise we reject it as ambiguous to avoid silent errors.
            const parts = t.split(",");
            if (parts.length === 2 && parts[1].length === 3) {
                // e.g. "1,234" → 1234 (thousands separator)
                t = t.replace(",", "");
            } else {
                // e.g. "72,5" in en-US mode → ambiguous, return NaN
                return NaN;
            }
        }
        // if only dots: that's fine, dot is the decimal separator
    }
...
function readNumber(id, name, min = -Infinity) {
    ...
    raw = raw.replace(/\s/g, ""); // Clean all whitespace/tabs/newlines from Excel
...
function parseVector(text) {
    // Split ONLY by semicolons, spaces, tabs, and newlines — NEVER by comma
    ...
    if (rejected.length > 0 && values.length > 0) {
        const sepLabel = decimalSeparator === "," ? "vírgula (,)" : "ponto (.)";
        throw new Error(
            `Os seguintes valores não puderam ser interpretados com o separador decimal "${sepLabel}": ${rejected.join(", ")}. ` +
            `Verifique a configuração do separador decimal na barra lateral.`
        );
    }
    return values;
}
...
function parseMatrix(text) {
    ...
    if (allRejected.length > 0 && matrix.length > 0) {
        const sepLabel = decimalSeparator === "," ? "vírgula (,)" : "ponto (.)";
        throw new Error(
            `Os seguintes valores não puderam ser interpretados com o separador decimal "${sepLabel}": ${allRejected.join(", ")}. ` +
            `Verifique a configuração do separador decimal na barra lateral.`
        );
    }
    return matrix;
...
function parseGroups(text) {
    ...
    if (trimmed.includes("\t")) {
        // Excel tab-separated copy-paste detected!
        const rows = trimmed.split(/\n+/).map(line => line.split("\t"));
        const numCols = Math.max(...rows.map(r => r.length));
        if (numCols > 1) {
            const firstRow = rows[0];
            const isHeader = firstRow.some(cell => {
                const val = normalizeToken(cell);
                return cell.trim() !== "" && !Number.isFinite(val);
            });
            const dataRows = isHeader ? rows.slice(1) : rows;
            const groupNames = isHeader 
                ? firstRow.map((cell, idx) => cell.trim() || `Grupo ${idx + 1}`)
                : Array.from({ length: numCols }, (_, idx) => `Grupo ${idx + 1}`);
            ...
            for (let j = 0; j < numCols; j++) {
                ...
                for (let i = 0; i < dataRows.length; i++) {
                    if (dataRows[i][j] !== undefined) {
                        const val = normalizeToken(dataRows[i][j]);
                        if (Number.isFinite(val)) {
                            values.push(val);
                        }
                    }
                }
                groups.push({ name: groupNames[j], values });
            }
            // Validate all groups
            groups.forEach(g => validateVector(g.values, g.name, 2));
            return groups.filter(g => g.values.length > 0);
        }
    }
    
    // Fallback to standard line-by-line format
...
// Inject settings and grouped links in sidebar dynamically
const sidebar = document.querySelector(".sidebar");
if (sidebar) {
    const path = window.location.pathname;
    const activePage = decodeURIComponent(path).split("/").pop() || "index.html";

    const links = [
        { type: "header", text: "Geral" },
        { text: "Descritivas", href: "descritivas.html" },
        { text: "Tamanho amostral", href: "amostra.html" },
        { type: "header", text: "Paramétricos" },
        { text: "Teste t", href: "teste-t.html" },
        { text: "ANOVA", href: "anova.html" },
        { text: "Correlação", href: "correlacao.html" },
        { text: "Regressão linear", href: "regressao-linear.html" },
        { text: "Regressão logística", href: "regressao-logistica.html" },
        { type: "header", text: "Não Paramétricos" },
        { text: "Não paramétricos", href: "nao-parametricos.html" },
        { text: "Qui-quadrado", href: "qui-quadrado.html" },
        { text: "Epidemiologia 2x2", href: "epidemiologia.html" },
        { text: "Alfa de Cronbach", href: "cronbach.html" }
    ];
    ...
    let html = `
      <div class="sidebar-settings">
        ...
        <label class="settings-label">
          ...
          <select id="decimal-separator-select">
            <option value=",">Vírgula (5,5)</option>
            <option value=".">Ponto (5.5)</option>
          </select>
        </label>
    ...
    links.forEach(item => {
        if (item.type === "header") {
            html += `<div class="sidebar-group-title">${item.text}</div>`;
        } else {
            const isActive = activePage === item.href;
            html += `<a href="${item.href}" class="${isActive ? "active" : ""}">${item.text}</a>`;
        }
    });

    sidebar.innerHTML = html;

    const select = document.getElementById("decimal-separator-select");
    if (select) {
        select.value = decimalSeparator;
        select.addEventListener("change", (e) => {
            const val = e.target.value;
            localStorage.setItem("decimalSeparator", val);
            decimalSeparator = val;
            updateHintsAndExamples();
        });
    }

    // Apply initial hints/examples on page load
    updateHintsAndExamples();
}

/**
 * Dynamically update hint texts and example data in textareas
 * based on the current decimalSeparator setting.
 */
function updateHintsAndExamples() {
    const sep = decimalSeparator;
    const decEx = sep === "," ? "72,5" : "72.5";
    const pairEx = sep === "," ? "2; 5,8" : "2; 5.8";

    // Update all hint elements
    document.querySelectorAll(".hint[data-hint-key]").forEach(el => {
        const key = el.dataset.hintKey;
        if (key === "vector") {
            el.textContent = `Use espaço, ponto e vírgula ou quebra de linha para separar os valores (ex: ${decEx}; 75; 70).`;
        } else if (key === "anova") {
            el.textContent = `Formato: Nome do grupo: valores. Use espaço ou ponto e vírgula para separar números (ex: Controle: ${decEx}; 75; 70).`;
        } else if (key === "pairs") {
            el.textContent = `Use espaço ou ponto e vírgula como separador (ex: ${pairEx}).${el.dataset.hintExtra ? " " + el.dataset.hintExtra : ""}`;
        } else if (key === "ttest") {
            el.textContent = `Use números separados por espaço, ponto e vírgula ou quebra de linha (ex: ${decEx}; 14; 13). No teste pareado, informe as diferenças já calculadas.`;
        } else if (key === "np") {
            el.textContent = `Use números separados por espaço, ponto e vírgula ou quebra de linha (ex: ${decEx}; 14; 13). Para Wilcoxon pareado, informe as diferenças pareadas no primeiro campo.`;
        }
    });

    // Inject or update a decimal-info-badge in each .tool section
    document.querySelectorAll(".tool").forEach(tool => {
        let badge = tool.querySelector(".decimal-info-badge");
        if (!badge) {
            badge = document.createElement("div");
            badge.className = "decimal-info-badge";
            const heading = tool.querySelector(".tool-heading");
            if (heading) {
                heading.after(badge);
            } else {
                tool.prepend(badge);
        ...
        if (sep === ",") {
            badge.innerHTML = `<strong>Separador decimal: vírgula (,)</strong> &mdash; Exemplo: 72,5 &nbsp;|&nbsp; Separadores de valores: <code>;</code> espaço ou quebra de linha`;
        } else {
            badge.innerHTML = `<strong>Separador decimal: ponto (.)</strong> &mdash; Exemplo: 72.5 &nbsp;|&nbsp; Separadores de valores: <code>;</code> espaço ou quebra de linha`;
        }
    });
}

// Global Excel Paste interceptor for two-group fields
document.addEventListener("paste", (event) => {
    ...
    if (pastedText.includes("\t")) {
        let siblingId = null;
        if (target.id === "ttest-raw-1") {
            const typeSelect = $("ttest-type");
            if (typeSelect && typeSelect.value === "independent") {
                siblingId = "ttest-raw-2";
            }
        } else if (target.id === "np-values-1") {
            const typeSelect = $("np-type");
            if (typeSelect && typeSelect.value === "mann") {
                siblingId = "np-values-2";
            }
        }
        ...
        if (siblingId) {
            ...
            if (sibling) {
                ...
                lines.forEach(line => {
                    const parts = line.split("\t");
                    if (parts[0] !== undefined) col1.push(parts[0].trim());
                    if (parts[1] !== undefined) col2.push(parts[1].trim());
                });

                if (col1.length > 0 && col2.length > 0) {
                    event.preventDefault();

                    // Use normalizeToken to detect headers
                    const hasHeader = !Number.isFinite(normalizeToken(col1[0])) || !Number.isFinite(normalizeToken(col2[0]));

                    const startIdx = hasHeader ? 1 : 0;
                    // Join with semicolons to avoid any comma ambiguity
                    const val1 = col1.slice(startIdx).join("; ");
                    const val2 = col2.slice(startIdx).join("; ");
